...
### Changed
- **Rebuilt the browser interface from scratch** as a single, reliable flow:
  a clean form that accepts typed paths / SMILES / names *and* file uploads, a
  dedicated "running" progress page with auto-refresh, and a full results page
  (best pose, ranked poses, ligand properties, predicted interactions, output
  paths). The old two-step form should no longer appear to "do nothing".

### Fixed
- **Web runs could hang permanently at the docking stage** when launched from
  the server (the page looked broken / never produced results). Vina is now
  spawned with `stdin=DEVNULL` so it no longer blocks on an inherited,
  unusable console handle on Windows.
- **Pipeline logging no longer destroys the framework's loggers**: when a run
  executes inside the web server the logger attaches its own console handler
  instead of clearing every handler on the root logger (which interfered with
  uvicorn's own logging setup).
- Guided CLI verified end-to-end with validated prompts.
